...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		...
		<i class="cat-icon"><?php echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'twentyfourteen' ) ); ?></i>
	...
	<footer class="entry-meta bottom">
		<?php
			...
			if ( ! tdpersona_categorized_blog() ) {
				...
				if ( '' != $tag_list ) {
						$meta_text = __( '这篇文章发表在 %1$s，标签有 %2$s，Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'tdpersona' );
				} else {
					$meta_text = __( '这篇文章发表在 %1$s，Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'tdpersona' );
				}
